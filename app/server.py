...
def create_app(db_path: str | None = None) -> Flask:
    ...
    def create_entry(branch_id: str):
        data = parse_json()
        kind = data.get("kind")
        payload = data.get("payload", {})
        parent_refs = data.get("parent_refs")
        entry_id = data.get("entry_id")  # 可选，测试/导入时指定草案编号
        if not isinstance(payload, dict):
            raise StoreError("INVALID_PAYLOAD", "payload 必须是对象",
                             status=400)
        ...
        if entry_id is not None and not isinstance(entry_id, str):
            raise StoreError("INVALID_ENTRY_ID",
                             "entry_id 必须是字符串", status=400)
        entry = store.create_entry(
            branch_id, kind, payload, parent_refs, entry_id=entry_id)
    ...
    def publish_branch(branch_id: str):
        ...
        if not operation_id or not isinstance(operation_id, str):
            raise StoreError(
                "OPERATION_ID_REQUIRED",
                "发布必须携带非空操作标识 operation_id", status=400)
